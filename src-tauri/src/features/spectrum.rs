...
pub struct SpectrumStop {
    pub stop: Arc<AtomicBool>,
}
pub struct SpectrumConfig {
    pub fft_size: AtomicUsize,
    pub columns: AtomicUsize,
}
...
impl SpectrumSettings {
    fn normalized(self) -> Self {
        ...
        Self {
            ...
            columns: self
                .columns
                .clamp(SPECTRUM_COLUMNS_MIN, SPECTRUM_COLUMNS_MAX),
        }
    }
}
...
pub fn set_spectrum_fft_size(app: tauri::AppHandle, size: usize) -> Result<(), String> {
    ...
    persist_spectrum_settings(SpectrumSettings {
        fft_size: size,
        columns,
    })?;
        ...
        if let Err(e) = run_wasapi_loopback(stop_flag.clone(), app_handle2.clone()) {
            ...
            loop {
                ...
                for i in 0..bins {
                    ...
                    let base = (t * 2.0 + x * 10.0).sin().abs() * (1.0 - x).powf(0.6)
...
fn run_wasapi_loopback(stop: Arc<AtomicBool>, app: tauri::AppHandle) -> Result<(), String> {
    unsafe {
        ...
        // 创建设备枚举器
        let mmdev_enum: IMMDeviceEnumerator =
        ...
        if wtag == WAVE_FORMAT_EXTENSIBLE_TAG {
            ...
            let wfex: &WAVEFORMATEXTENSIBLE =
                &*(mix_format as *const _ as *const WAVEFORMATEXTENSIBLE);
            ...
            let sub: windows::core::GUID =
                std::ptr::read_unaligned(std::ptr::addr_of!(wfex.SubFormat));
            ...
            // 对于可扩展格式，优先使用有效位数
            let vbps: u16 =
                std::ptr::read_unaligned(std::ptr::addr_of!(wfex.Samples.wValidBitsPerSample));
            if vbps != 0 {
                valid_bits = vbps;
            }
        }
        ...
        let mut fft_size = cfg_state
            .fft_size
            .load(Ordering::Relaxed)
            .clamp(FFT_SIZE_MIN, FFT_SIZE_MAX);
        ...
        let role_name = if role_used == eMultimedia {
            "multimedia"
        } else if role_used == eConsole {
            "console"
        } else {
            "communications"
        };
        ...
        loop {
            ...
            let mut packet_len: u32 = match capture.GetNextPacketSize() {
                Ok(n) => n,
                Err(_) => 0,
            };
            if packet_len == 0 {
                    ...
                    if silent {
                        ...
                        match bytes_per_sample {
                            1 => {
                                // 8-bit unsigned
                                let raw =
                                    std::slice::from_raw_parts(data_ptr as *const u8, samples);
                            ...
                            3 => {
                                ...
                                let raw =
                                    std::slice::from_raw_parts(data_ptr as *const u8, samples * 3);
                                for i in 0..samples {
                                    ...
                                    // 符号扩展 24-bit -> 32-bit
                                    let v32: i32 = if (v24 & 0x0080_0000) != 0 {
                                        (v24 | 0xFF00_0000) as i32
                                    } else {
                                        v24 as i32
                                    };
                            ...
                            4 => {
                                // 32-bit container（validBits 可能为 24 或 32）
                                let raw =
                                    std::slice::from_raw_parts(data_ptr as *const i32, samples);
            ...
            if acc.len() >= per_ch_needed * channels {
                ...
                // 复数输入
                let mut buf: Vec<rustfft::num_complex::Complex<f32>> = mono
                    .into_iter()
                ...
                let mut min_bin =
                    ((20.0_f32 * fft_size as f32) / (sample_rate as f32)).ceil() as usize;
                if min_bin < 1 {
                    min_bin = 1;
                }
                ...
                for i in 0..cols {
                    ...
                    let mut end =
                        ((ln_min + p1 * span).exp() as usize).clamp(start + 1, max_bin + 1);
                    if end <= start {
                        end = start + 1;
                    }
                    let mut mx = 0.0f32;
                    for j in start..end.min(half) {
                        mx = mx.max(mags[j]);
                    }
                    cols_out[i] = mx;
                }
                ...
                if last_emit.elapsed() > std::time::Duration::from_secs(2) {
                    ...
                    if last_status != msg {
                        let _ = app.emit("spectrum:status", msg.clone());
                        last_status = msg;
                    }
                }
